...
├─ Settings\
│  └─ Settings.ini              控制面板（亮色主题，左导航 8 分页，命名遵循约定）
├─ @Resources\
│  ├─ Configs\
...
│  ├─ Scripts\
│  │  ├─ Lunar.lua             农历/节气/节日离线计算（1900-2100 数据表）
...
- **渲染管线**：`Calendar.lua` 每天渲染一次，流程为
  `LoadConfig → 星期表头 → 背景 → 月份水印 → 网格(清空/填日期/今日/标记) → 封面`。
  设置变更通过 Refresh 触发全量重渲染，无增量状态残留。
...
**新增一个数值型设置**
1. 两个 .inc 加键；2. `INT_RULES` 加 `MyKey = {最小值, 最大值, 默认值}`；
3. 面板加输入框（仿 `BgRoundedSize*`），命令 `SetInt('MyKey', '$UserInput$')`。
